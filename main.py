...
"""
╔═══════════════════════════════════════════════════════════╗
║  🌟 XTC HOSTING PANEL — Maximum Security Edition v6.5 🌟  ║
║  Sandboxed • Scanned • Quota-enforced Terminal           ║
╚═══════════════════════════════════════════════════════════╝
"""
...
from handlers.premium_emoji import btn, set_viewer_premium

def _is_prem(user) -> bool:
    """True if Telegram user has Premium subscription."""
    try:
...
try:
    from security import (
        ...
        get_terminal, shutdown_terminal, quota_bar, dir_size_mb,
    )
    _SECURITY_OK = True
    print("[security] ✅ loaded (guard + sandbox + terminal)")
...
def show_admin_panel(uid):
    ...
    safe_send(uid,
        ...
        f"🛡 Security: {'🟢 ACTIVE' if _SECURITY_OK else '🔴 DISABLED'}\n"
        f"━━━━━━━━━━━━━━━━━━━━",
...
def show_user_info(admin_uid, target_uid):
    ...
    m.add(
        btn("Ban",   style="danger",  emoji_key="ban",   callback_data=f"adm_ban_direct:{target_uid}"),
        btn("Unban", style="success", emoji_key="check", callback_data=f"adm_unban_direct:{target_uid}"),
    )
...
# ═══════════════════════════════════════════════════
#  🔐 ASYNC SECURITY SCAN HELPERS
#  Fast sync scan BEFORE approval decision.
#  Only CRITICAL malware → auto-block + admin msg (with file) — NO auto-ban.
#  Everything else (even high soft-score) → SAFE path → approval / auto-run.
# ═══════════════════════════════════════════════════
...
def _sha256_sources(root: str) -> list:
    """
    Hash every source file under root (sorted).
    Returns list of (rel_path, sha256). Also returns combined hash of all sources.
    """
    root_p = Path(root)
    pairs = []
    for f in sorted(root_p.rglob('*')):
        ...
        if digest:
            pairs.append((str(f.relative_to(root_p)), digest))
    return pairs


def _any_hash_blocked(hashes):
    """Return first blocked hash record if any hash is blacklisted."""
    for h in hashes:
        if not h:
            continue
        rec = db.is_hash_blocked(h)
        if rec:
            return rec
    return None


def _blacklist_hashes(hashes, reason='', filename='', user_id=None):
    for h in hashes:
        if h:
            db.add_blocked_hash(h, reason=reason, filename=filename, user_id=user_id)


REJECT_SCORE = 50          # critical (stealer/backdoor) — matches guard has_critical


def _notify_admins_blocked(uid, score, path, threats_text, file_to_send=None):
    """Send block alert to all admins, optionally attaching the blocked file."""
    caption = (
        f"🚨 <b>MALICIOUS UPLOAD BLOCKED</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        ...
        f"⚠️ Threats:\n{threats_text}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"✅ Files auto-deleted.\n"
        f"ℹ️ Ban requires admin action (no auto-ban)."
    )
    for aid in state.admin_ids:
        try:
            if file_to_send and os.path.isfile(file_to_send):
                with open(file_to_send, 'rb') as fh:
                    bot.send_document(aid, fh, caption=caption)
            else:
                safe_send(aid, caption)
        except Exception:
            try:
                safe_send(aid, caption)
            except Exception:
...
def _sync_scan_and_block_zip(extract_dir, uid, chat_id, progress_msg_id=None):
    """
    Returns True  → SAFE / soft-risk → caller continues to approval/run
    Returns False → CRITICAL malware → files deleted, admin notified (with sample)
    Never auto-bans. Ban is always admin decision.
    Hash blacklist prevents re-upload after rename.
    """
    # ── Hash blacklist check (all source files) ──
    pairs = _sha256_sources(extract_dir)
    hashes = [h for _, h in pairs]
    blocked = _any_hash_blocked(hashes)
    if blocked:
        reason = blocked.get('reason') or 'previously blocked malware'
        bname = blocked.get('filename') or '?'
        if progress_msg_id:
            try:
                safe_edit(
                    f"🚫 <b>BLOCKED — KNOWN MALWARE</b>\n"
                    f"━━━━━━━━━━━━━━━━━━━━\n\n"
                    f"📄 Match: <code>{bname}</code>\n"
                    f"🔐 Hash: <code>{(blocked.get('hash') or '')[:16]}…</code>\n"
                    f"⚠️ {reason}\n"
                    f"━━━━━━━━━━━━━━━━━━━━\n"
                    f"This code was blocked before (even if renamed/repacked).",
        ...
        for aid in state.admin_ids:
            try:
                safe_send(aid,
                    f"🚫 <b>KNOWN MALWARE RE-UPLOAD (ZIP)</b>\n"
                    f"👤 <code>{uid}</code>\n"
                    f"📄 Match: <code>{bname}</code>\n"
                    f"⚠️ {reason}")
            except Exception:
    ...
    try:
        if not _SECURITY_OK:
            return True
        result = scan_directory_recursive(extract_dir, reject_threshold=REJECT_SCORE)
    except Exception as e:
        logger.warning(f"sync scan zip error: {e}")
        return True

    worst = result.get("worst") or {}
    score = worst.get("risk_score", 0)
    has_critical = worst.get("has_critical", False)

    # Soft / no critical → always allow (approval flow handles the rest)
    if result.get("ok") or not has_critical or score < REJECT_SCORE:
        if progress_msg_id:
            try:
                safe_edit(
                    f"✅ <b>Scan Complete</b>\n"
                    ...
                    f"📈 Risk: {score}/100\n"
                    f"🛡 Sandbox active at runtime",
                    chat_id, progress_msg_id)
            except Exception:
                pass
        return True

    # ═══ CRITICAL only → delete + notify admin WITH file ═══
    path = worst.get("path", "?")
    threats = worst.get("all_threats", [])[:6]
    threats_text = "\n".join(f"  • <code>{t[:70]}</code>" for t in threats)

    # Pick a sample file to send admin (the threat file if possible)
    sample = None
    try:
        cand = Path(extract_dir) / path if path and path != "?" else None
        if cand and cand.is_file() and cand.stat().st_size < 5 * 1024 * 1024:
            sample = str(cand)
        else:
            for f in Path(extract_dir).rglob('*.py'):
                if f.stat().st_size < 5 * 1024 * 1024:
                    sample = str(f); break
    except Exception:
        pass
    ...
    if progress_msg_id:
        try:
            safe_edit(
                f"🚫 <b>CRITICAL THREAT BLOCKED — FILE DELETED</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━\n\n"
                f"📊 Risk Score: <b>{score}/100</b>\n"
                f"📄 Threat File: <code>{path}</code>\n\n"
                f"⚠️ <b>Detected Threats:</b>\n{threats_text}\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"Your files were deleted. Admin has been notified.",
                chat_id, progress_msg_id)
        except Exception:
            pass

    _notify_admins_blocked(uid, score, path, threats_text, file_to_send=sample)

    shutil.rmtree(extract_dir, ignore_errors=True)
    try:
        for b in db.get_bots(uid):
            if b.get('file_path') == extract_dir:
                db.del_bot(b['bot_id'])
                break
    except Exception:
        pass

    # Permanent hash ban — pairs was computed at start (before delete)
    _blacklist_hashes([h for _, h in pairs], reason=f"critical malware score={score}",
                      filename=str(path), user_id=uid)

    forward_error("SECGUARD_REJECT", f"User {uid}: score={score} path={path}", uid)
    logger.warning(f"[security] critical block user {uid} score={score}")
    return False


def _sync_scan_and_block_file(file_path, bot_dir, uid, chat_id, progress_msg_id=None):
    """Same policy as zip version for single-file uploads."""
    # ── Hash blacklist (rename cannot bypass) ──
    fhash = _sha256_file(file_path)
    blocked = _any_hash_blocked([fhash])
    if blocked:
        ...
        if progress_msg_id:
            try:
                safe_edit(
                    ...
                    f"━━━━━━━━━━━━━━━━━━━━\n\n"
                    f"📄 <code>{fname}</code>\n"
                    f"🔐 Hash: <code>{fhash[:16]}…</code>\n"
                    f"⚠️ {reason}\n"
                    f"━━━━━━━━━━━━━━━━━━━━\n"
                    f"This file was blocked before (even if renamed).",
                    chat_id, progress_msg_id)
            except Exception:
                pass
        ...
        for aid in state.admin_ids:
            try:
                safe_send(aid,
                    f"🚫 <b>KNOWN MALWARE RE-UPLOAD</b>\n"
                    f"👤 <code>{uid}</code>\n"
                    f"📄 <code>{fname}</code>\n"
                    f"🔐 <code>{fhash[:24]}…</code>\n"
                    f"⚠️ {reason}")
            except Exception:
                pass
        return False
    ...
    try:
        ...
        logger.warning(f"sync scan file error: {e}")
        return True

    score = result.get("risk_score", 0)
    has_critical = result.get("has_critical", False)

    if not has_critical or score < REJECT_SCORE:
        if progress_msg_id:
            try:
                safe_edit(
                    f"✅ <b>Scan Complete</b>\n"
    ...
    threats = result.get("all_threats", [])[:6]
    threats_text = "\n".join(f"  • <code>{t[:70]}</code>" for t in threats)
    fname = os.path.basename(file_path)
    ...
    sample = file_path if (os.path.isfile(file_path) and os.path.getsize(file_path) < 5 * 1024 * 1024) else None
    _notify_admins_blocked(uid, score, fname, threats_text, file_to_send=sample)

    shutil.rmtree(bot_dir, ignore_errors=True)
    try:
        for b in db.get_bots(uid):
            if b.get('file_path') == bot_dir:
                db.del_bot(b['bot_id'])
                break
    except Exception:
        pass

    # Permanent hash ban — rename will not help
    _blacklist_hashes([fhash], reason=f"critical malware score={score}",
                      filename=fname, user_id=uid)

    forward_error("SECGUARD_REJECT", f"File from {uid}: score={score}", uid)
    logger.warning(f"[security] critical block file from {uid} score={score} hash={fhash[:12]}")
    return False


# ═══════════════════════════════════════════════════
#  🖥️ PER-BOT TERMINAL HELPERS
# ═══════════════════════════════════════════════════
def _term_dirs_and_quota(bd):
    """Return (bot_dir, user_dir, quota_mb) for a bot doc."""
    bot_dir = (Path(bd['file_path'])
               if os.path.isdir(bd['file_path'])
               else Path(user_folder(bd['user_id'])))
    user_dir = Path(user_folder(bd['user_id']))
    try:
        plan = db.get_plan(bd['user_id'])
        quota = int(plan.get('disk_mb', 100))
    except Exception:
        quota = 100
    return bot_dir, user_dir, quota


def _term_render(chat_id, uid, msg_id, bid):
    """Show terminal control panel with live quota."""
    if not _SECURITY_OK:
        return safe_edit("🖥 <b>Terminal unavailable</b>\n<i>security package not loaded.</i>",
                         chat_id, msg_id)
    bd = db.get_bot(bid)
    if not bd or (bd['user_id'] != uid and not state.is_admin(uid)):
        return safe_edit("❌ Bot not found or not yours.", chat_id, msg_id)

    bot_dir, user_dir, quota = _term_dirs_and_quota(bd)
    try:
        term = get_terminal(str(bid), bot_dir, user_dir, quota)
        used = term.usage_mb()
    except Exception as e:
        return safe_edit(f"❌ Terminal error: {str(e)[:150]}", chat_id, msg_id)

    pct = min(100, int(used / quota * 100)) if quota else 0
    bar = quota_bar(pct)

    try:
        sb_mode = sandbox_status().get('mode', '?')
    except Exception:
        sb_mode = 'rlimits-only'

    term_mode = 'simple' if getattr(term, 'simple', False) else 'interactive'
    text = (
        f"🖥 <b>Isolated Terminal</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"🤖 Bot: <code>{bd['bot_name'][:22]}</code>\n"
        f"🛡 Jail: <b>{sb_mode}</b> | Mode: <b>{term_mode}</b>\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"💾 Quota: <b>{used:.1f} / {quota} MB</b>\n"
        f"{bar} {pct}%\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"1️⃣ Tap <b>Run</b> then type a command\n"
        f"2️⃣ Tap <b>Output</b> to see result\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"<i>Allowed: ls cat python pip node npm git …</i>\n"
        f"<i>Blocked: sudo apt mount rm -rf / …</i>")
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        btn("Run",    style="success", emoji_key="play",  callback_data=f"term_run:{bid}"),
        btn("Output", style="primary", emoji_key="logs",  callback_data=f"term_out:{bid}"))
    kb.add(
        btn("Clear",  style="danger",  emoji_key="trash", callback_data=f"term_clear:{bid}"),
        btn("Back",   style="primary", emoji_key="back",  callback_data=f"bot_detail:{bid}"))
    safe_edit(text, chat_id, msg_id, reply_markup=kb)


def _term_show_output(chat_id, uid, msg_id, bid):
    """Read pending terminal output and display it."""
    if not _SECURITY_OK:
        return safe_edit("🖥 Terminal unavailable.", chat_id, msg_id)
    bd = db.get_bot(bid)
    if not bd or (bd['user_id'] != uid and not state.is_admin(uid)):
        return safe_edit("❌ Not yours.", chat_id, msg_id)
    bot_dir, user_dir, quota = _term_dirs_and_quota(bd)
    try:
        term = get_terminal(str(bid), bot_dir, user_dir, quota)
        output = term.read_output(wait=0.3)
    except Exception as e:
        return safe_edit(f"❌ {str(e)[:150]}", chat_id, msg_id)

    # Scrub secrets from output
    output = re.sub(r'\b\d{8,10}:[A-Za-z0-9_-]{33,}\b', '[BOT_TOKEN]', output)
    output = re.sub(r'mongodb(\+srv)?://[^\s]+', '[MONGO_URL]', output)
    output = re.sub(r'https://[^\s]*upstash[^\s]*', '[REDIS_URL]', output)
    if not output.strip():
    ...
    cap = (
        ...
        f"💾 {used:.1f}/{quota} MB\n"
        f"━━━━━━━━━━━━━━━━━━━━\n"
        f"<pre>{_html.escape(output[:3500])}</pre>")
    kb = types.InlineKeyboardMarkup(row_width=2)
    kb.add(
        btn("Run",    style="success", emoji_key="play",  callback_data=f"term_run:{bid}"),
        btn("Clear",  style="danger",  emoji_key="trash", callback_data=f"term_clear:{bid}"))
    kb.add(btn("Back", style="primary", emoji_key="back",
               callback_data=f"term_open:{bid}"))
    safe_edit(cap, chat_id, msg_id, reply_markup=kb)
...
def _process_github_download(uid, chat_id, msg_id):
    ...
    try:
        ...
        # ═══ Fast SYNC scan FIRST ═══
        _gh_progress(chat_id, msg_id, "🛡 <b>Files received.</b>\n<i>Scanning…</i>")
        is_safe = _sync_scan_and_block_zip(ed, uid, chat_id, progress_msg_id=msg_id)
        if not is_safe:
            github_sessions.pop(uid, None)
            return  # blocked + admin notified, no approval
        ...
        if approval_is_on():
            ...
            mk.add(
                btn("Start Now", style="success", emoji_key="play", callback_data=f"bot_start:{bid}"),
...
def handle_user_state(msg):
    ...
    try:
        if action == 'broadcast':
            ...
            try:
                ...
                for k, p in PLAN_LIMITS.items():
                    if k != 'free':
                        m.add(btn(p['name'], style="success", emoji_key="gem",
                                  callback_data=f"adm_setplan:{k}:{target}"))
                m.add(btn("Cancel", style="danger", emoji_key="cross", callback_data="menu_admin"))
                safe_reply(msg,
            ...
            if len(parts) >= 3:
                try:
                    ...
                    safe_reply(msg, f"❌ Error: {str(e)[:100]}")
            else:
                safe_reply(msg, "❌ Format: CODE TK MAX_USES\nEx: WELCOME100 100 50")
            state.clear_state(uid)
            ...
            for aid in state.admin_ids:
                ...
                m.add(btn(f"Reply #{tid}", style="primary", emoji_key="speech",
                          callback_data=f"adm_ticket_reply:{tid}"))
                safe_send(aid,
        ...
        # ═══ 🖥️ TERMINAL COMMAND INPUT ═══

        # ═══ 📦 PIP / PKG INSTALL ═══
        elif action == 'pip_install':
            bid = s.get('bid')
            state.clear_state(uid)
            bd = db.get_bot(bid) if bid else None
            if not bd or (bd['user_id'] != uid and not state.is_admin(uid)):
                return safe_reply(msg, "❌ Not yours!")
            raw = (msg.text or "").strip()
            if not raw:
                return safe_reply(msg, "❌ Empty package name.")
            # Only safe package specs: letters, digits, .-_==<>~, spaces between pkgs
            if not re.match(r'^[A-Za-z0-9._\-=<>~, ]{1,200}$', raw):
                return safe_reply(msg, "❌ Invalid characters in package name.")
            # Block flags / path injection
            low = raw.lower()
            for bad in ('--', ' -', '../', '/', '\\', ';', '|', '&', '`', '$',
                        'http://', 'https://', 'git+', 'file:'):
                if bad in low or bad in raw:
                    return safe_reply(msg, f"🚫 Blocked: {bad!r}")
            pkgs = [p for p in raw.split() if p]
            if not pkgs or len(pkgs) > 10:
                return safe_reply(msg, "❌ 1–10 packages at a time.")
            bot_dir = bd['file_path'] if os.path.isdir(bd['file_path']) else os.path.dirname(bd['file_path'])
            safe_reply(msg, f"📦 Installing: <code>{_html.escape(' '.join(pkgs))}</code>\n⏳ Please wait…")
            try:
                # Install into bot folder only (--target) so host stays clean
                target = os.path.join(bot_dir, '.deps')
                os.makedirs(target, exist_ok=True)
                cmd = [sys.executable, '-m', 'pip', 'install', '--disable-pip-version-check',
                       '--no-warn-script-location', '--target', target, *pkgs]
                r = subprocess.run(cmd, cwd=bot_dir, capture_output=True, text=True, timeout=180)
                out = ((r.stdout or '') + (r.stderr or ''))[-2500:]
                status = "✅ OK" if r.returncode == 0 else "⚠️ Finished with errors"
                kb = types.InlineKeyboardMarkup()
                kb.add(btn("Back", style="primary", emoji_key="back",
                           callback_data=f"bot_detail:{bid}"))
                safe_reply(msg,
                    f"{status}\n"
                    f"━━━━━━━━━━━━━━━━━━━━\n"
                    f"<pre>{_html.escape(out) or '(no output)'}</pre>",
                    reply_markup=kb)
            except subprocess.TimeoutExpired:
                safe_reply(msg, "⚠️ Install timed out (180s).")
            except Exception as e:
                forward_crash("pip_install", e, uid)
                safe_reply(msg, f"❌ Install error: {str(e)[:150]}")
...
def cmd_start(msg):
    uid = msg.from_user.id
    set_viewer_premium(bool(state.panel_is_premium))
    ...
    try:
        ...
        if not joined:
            send_force_sub(msg.chat.id, nj); return
        ...
        disk = pl.get('disk_mb', 100)

        welcome = (
            f"🌟 <b>XTC HOSTING PANEL</b> {BRAND_VER}\n"
...
def cmd_help(msg):
    ...
    try:
        ...
        forward_crash("cmd_help", e, msg.from_user.id)
...
def cmd_ping(msg):
    ...
    if m:
        ...
        safe_edit(f"🏓 <b>Pong!</b>\n\n⚡ Latency: {latency}ms\n⏱️ Uptime: {get_uptime()}\n"
                  f"🤖 Running: {rn} bots\n🛡 Security: {'🟢 ACTIVE' if _SECURITY_OK else '🔴 OFF'}\n{BRAND_FOOTER}",
...
def cmd_status(msg):
    ...
    try:
        ...
        if not bots_list:
            ...
            mk_empty.add(btn("Deploy Bot", style="success", emoji_key="send", callback_data="menu_deploy"))
            return safe_reply(msg,
        ...
        for b, r in running:
            ...
            m.add(btn(f"{b['bot_name'][:18]} #{b['bot_id']}",
                      style="success" if r else "primary",
                      emoji_key="green" if r else "robot",
...
# ═══════════════════════════════════════════════════
#  DOCUMENT HANDLER (with async security scan)
# ═══════════════════════════════════════════════════
...
def handle_doc(msg):
    ...
    try:
        ...
        if mx != -1 and cur >= mx:
            mk_lim = types.InlineKeyboardMarkup()
            mk_lim.add(btn("Upgrade", style="success", emoji_key="gem", callback_data="menu_sub"))
            return safe_reply(msg,
        ...
        if ext == 'zip':
            ...
            try:
                with zipfile.ZipFile(tp, 'r') as z:
                    ...
                    base_bn = bn
                    counter = 1
                    while os.path.exists(os.path.join(uf, bn)):
                        counter += 1
                        bn = f"{base_bn}_{counter}"
                    ed = os.path.join(uf, bn)
                    extract_path = os.path.realpath(ed)
                    for n in z.namelist():
                        ...
                        if not member_path.startswith(extract_path + os.sep) and member_path != extract_path:
                            if pm:
                                safe_edit("❌ Suspicious file paths in ZIP! Upload rejected.", msg.chat.id, pm.message_id)
                            os.unlink(tp); return
                    if os.path.exists(ed):
                        shutil.rmtree(ed, ignore_errors=True)
                ...
                # ═══ Approval ON → skip scan; OFF → scan then run ═══
                if not approval_is_on():
                    if pm:
                        safe_edit(
                            f"🛡 <b>Files received.</b>\n"
                            f"<i>Scanning...</i>",
                            msg.chat.id, pm.message_id)
                    is_safe = _sync_scan_and_block_zip(
                        ed, uid, msg.chat.id,
                        progress_msg_id=pm.message_id if pm else None)
                    if not is_safe:
                        return  # already deleted + admin notified
                else:
                    if pm:
                        safe_edit(
                            f"📥 <b>Files received.</b>\n"
                            f"<i>Sending for admin approval...</i>",
                            msg.chat.id, pm.message_id)
                ...
                if not entry:
                    ...
                    if pm:
                        safe_edit(err, msg.chat.id, pm.message_id, reply_markup=back_btn())
                    return
                ...
                if approval_is_on():
                    ...
                    for aid in state.admin_ids:
                        try:
                            ...
                            forward_error("APPROVAL_SEND", ex, aid)
                    return
                ...
                mk.add(
                    btn("Start Now", style="success", emoji_key="play", callback_data=f"bot_start:{bid}"),
                    btn("My Bots",   style="primary", emoji_key="robot", callback_data="menu_mybots"))
                mk.add(btn("Re-detect", style="primary", emoji_key="search", callback_data=f"bot_redetect:{bid}"))
                if pm:
                    safe_edit(f"✅ <b>ZIP DEPLOYED!</b>\n\n📦 <code>{bn[:20]}</code>\n🆔 Bot ID: #{bid}\n\n🔍 <b>Detection:</b>\n{report}\n🛡 Scan: skipped (approval ON)",
                ...
                for aid in state.admin_ids:
                    try:
                        ...
                        forward_error("ADMIN_FORWARD_ZIP", ex, aid)
            ...
            bn_base = fn.rsplit('.', 1)[0]
            bn = bn_base
            counter = 1
            while os.path.exists(os.path.join(uf, bn)):
                counter += 1
                bn = f"{bn_base}_{counter}"
            bot_dir = os.path.join(uf, bn)
            os.makedirs(bot_dir, exist_ok=True)
            file_path = os.path.join(bot_dir, fn)
            ...
            if not approval_is_on():
                if pm:
                    safe_edit(
                        ...
                        f"<i>Scanning...</i>",
                        msg.chat.id, pm.message_id)
                is_safe = _sync_scan_and_block_file(
                    file_path, bot_dir, uid, msg.chat.id,
                    progress_msg_id=pm.message_id if pm else None)
                if not is_safe:
                    return  # already deleted + admin notified
            else:
            ...
            bid = db.add_bot(uid, bn, bot_dir, fn, ext, '', fs, 'exact')
            ...
            mk = types.InlineKeyboardMarkup(row_width=2)
            mk.add(
                btn("Run Now",  style="success", emoji_key="play",  callback_data=f"bot_start:{bid}"),
                btn("My Bots",  style="primary", emoji_key="robot", callback_data="menu_mybots"))
            if pm:
                safe_edit(
                    f"✅ <b>FILE UPLOADED!</b>\n\n📄 <code>{fn[:25]}</code>\n"
            ...
            for aid in state.admin_ids:
                try:
                    ...
                    forward_error("ADMIN_FORWARD_FILE", ex, aid)
        else:
...
def on_successful_payment(msg):
    ...
    try:
        ...
        mk_star = types.InlineKeyboardMarkup(row_width=2)
        mk_star.add(
            btn("Deploy Bot", style="success", emoji_key="send", callback_data="menu_deploy"),
            btn("Menu",       style="primary", emoji_key="home", callback_data="go_home"))
        ...
        safe_send(uid,
            ...
            f"💾 Disk: {p.get('disk_mb', 100)} MB\n"
            f"🎉 Your plan is now active!\n"
            f"━━━━━━━━━━━━━━━━━━━━",
            reply_markup=mk_star)
...
def handle_callback(call):
    ...
    # Panel emoji mode follows the BOT-OWNER account (not the viewer)
    set_viewer_premium(bool(state.panel_is_premium))
    ...
    try:
        ...
        # ═══ 🖥️ TERMINAL ═══
        elif data.startswith("term_open:"):
            safe_answer(call.id, "Terminal disabled for security", show_alert=True)

        
        elif data.startswith("term_run:"):
            safe_answer(call.id, "Terminal disabled", show_alert=True)

        
        elif data.startswith("term_out:"):
            safe_answer(call.id, "Terminal disabled", show_alert=True)

        
        elif data.startswith("term_clear:"):
            safe_answer(call.id, "Terminal disabled", show_alert=True)
            ...
            if not bots_list:
                m = types.InlineKeyboardMarkup(row_width=2)
                m.add(btn("Deploy Bot", style="success", emoji_key="send", callback_data="menu_deploy"))
                m.add(btn("Main Menu",  style="primary", emoji_key="home", callback_data="go_home"))
            ...
            for b in bots_list:
                ...
                m.add(btn(f"{st_icon} {b['bot_name'][:15]} — #{b['bot_id']}",
                          style="success" if (r and appr == 'approved') else "primary",
                          emoji_key="green" if (r and appr == 'approved') else "robot",
                          callback_data=f"bot_detail:{b['bot_id']}"))
            m.add(btn("Deploy New Bot", style="success", emoji_key="send", callback_data="menu_deploy"))
            m.add(btn("Main Menu",      style="primary", emoji_key="home", callback_data="go_home"))
        ...
        # ═══ DEPLOY ═══
            ...
            if mx != -1 and cur >= mx:
                m = types.InlineKeyboardMarkup()
                m.add(btn("Upgrade Plan", style="success", emoji_key="gem",  callback_data="menu_sub"))
                m.add(btn("Main Menu",    style="primary", emoji_key="home", callback_data="go_home"))
        ...
        # ═══ SUBSCRIPTION ═══
            ...
            t = (
                ...
                f"💾 Disk: {db.get_plan(uid).get('disk_mb', 100)} MB\n"
                f"━━━━━━━━━━━━━━━━━━━━\n\n"
            ...
            for k, p in PLAN_LIMITS.items():
                ...
                t += (f"{p['name']}\n"
                      f"  🤖 {slots} bots | 💾 {p.get('disk_mb', 100)}MB\n"
                      f"  💰 {p['price']} BDT/month\n\n")
            t += "━━━━━━━━━━━━━━━━━━━━\n👇 <b>Select a plan:</b>"
        ...
        # ═══ ADMIN PROMO ═══
            ...
            for p in promos[:15]:
                ...
                if ptype == 'wallet':
                    val = f"💵 {p.get('wallet_amount', 0)} Tk"
                else:
                    val = f"💰 {p.get('discount_pct', 0)}%"
            ...
            m = types.InlineKeyboardMarkup()
            m.add(btn("Cancel", style="danger", emoji_key="cross", callback_data="adm_promo"))
            safe_edit(
        ...
        # ═══ FILE APPROVAL ═══
            ...
            if p_cid and p_mid:
                try:
                    safe_delete(p_cid, p_mid)
                except Exception:
                    pass

            mk_u = types.InlineKeyboardMarkup(row_width=1)
            mk_u.add(btn("Start Now", style="success", emoji_key="play", callback_data=f"bot_start:{bid}"))
            mk_u.add(btn("My Bots",   style="primary", emoji_key="robot", callback_data="menu_mybots"))
            ...
            # Hash-ban so rename/re-upload fails
            fpath = bd.get('file_path') or ''
            try:
                if os.path.isdir(fpath):
                    pairs = _sha256_sources(fpath)
                    _blacklist_hashes([h for _, h in pairs],
                        reason=f"admin rejected bot #{bid}",
                        filename=bd.get('bot_name', ''), user_id=bd.get('user_id'))
                elif os.path.isfile(fpath):
                    _blacklist_hashes([_sha256_file(fpath)],
                        reason=f"admin rejected bot #{bid}",
                        filename=bd.get('bot_name', ''), user_id=bd.get('user_id'))
                entry = bd.get('entry_file') or ''
                ep = os.path.join(fpath, entry) if fpath and entry else ''
                if ep and os.path.isfile(ep):
                    _blacklist_hashes([_sha256_file(ep)],
                        reason=f"admin rejected bot #{bid}",
                        filename=entry, user_id=bd.get('user_id'))
                if fpath and os.path.isdir(fpath):
                    shutil.rmtree(fpath, ignore_errors=True)
            except Exception as ex:
                logger.warning(f"hash ban on reject failed: {ex}")

            db.set_approval_status(bid, 'rejected')
            db.admin_log(uid, 'reject_file', bd['user_id'], f"#{bid}")
            safe_answer(call.id, "❌ File rejected + hash banned!")
            ...
            if p:
                try:
                    ...
                    for ent in msgs.get('admin_msgs', []):
                        try:
                            if ent.get('cid') == chat_id and ent.get('mid') == msg_id:
                                continue
                ...
                mk_p = types.InlineKeyboardMarkup(row_width=2)
                mk_p.add(
                    btn("Deploy Bot", style="success", emoji_key="send", callback_data="menu_deploy"),
                    btn("My Plan",    style="primary", emoji_key="gem",  callback_data="menu_sub"))
                safe_send(p['user_id'],
        ...
        # ═══ ADMIN PANEL ═══
            ...
            m = types.InlineKeyboardMarkup(row_width=2)
            m.add(
                btn("Copy Link",    style="primary", emoji_key="logs",   callback_data=f"ref_copy:{rc}"),
                btn("My Referrals", style="primary", emoji_key="users",  callback_data="ref_list"),
            )
            m.add(btn("Leaderboard", style="primary", emoji_key="trophy", callback_data="ref_board"))
            m.add(btn("Main Menu",   style="primary", emoji_key="home",   callback_data="go_home"))
            safe_edit(t, chat_id, msg_id, reply_markup=m)
            ...
            role = get_role_label(uid)
            disk = db.get_plan(uid).get('disk_mb', 100)
            t = (
                f"📊 <b>STATISTICS</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━\n\n"
                f"🖥 <b>System</b>\n"
            ...
            for b, _ in act:
                ...
                m.add(btn(f"{b['bot_name'][:15]}",
                          style="primary", emoji_key="gear",
                          callback_data=f"bot_detail:{b['bot_id']}"))
            if not act:
                t += "No bots running!"
            m.add(btn("Main Menu", style="primary", emoji_key="home", callback_data="go_home"))
        ...
        # ═══ BOT DETAIL ═══
            ...
            bd_quota = 100
            try:
                bd_dir, bd_ud, bd_quota = _term_dirs_and_quota(bd)
                # Per-bot folder size (not whole user dir)
                target = str(bd_dir) if bd_dir and Path(bd_dir).exists() else str(bd_ud)
                if _SECURITY_OK:
                    disk_used = dir_size_mb(target)
                else:
                    # fallback without security module
                    total = 0
                    for root, dirs, files in os.walk(target, followlinks=False):
                        dirs[:] = [d for d in dirs if d not in (
                            '.tmp_run', '.home', '__pycache__', '.git')]
                        for f in files:
                            try:
                                total += os.path.getsize(os.path.join(root, f))
                            except OSError:
                                pass
                    disk_used = round(total / (1024 * 1024), 2)
            except Exception:
                pass
            t = (
                ...
                f"📂 Disk: {disk_used:.1f}/{bd_quota} MB\n"
        ...
        # ═══ BOT START ═══
            ...
            is_admin_user = (bd['user_id'] == OWNER_ID) or state.is_admin(bd['user_id'])
            if not is_admin_user and not db.is_active(bd['user_id']):
            ...
            # Shutdown terminal too
            try:
                shutdown_terminal(str(bid))
            except Exception:
                pass
            ...
            m = types.InlineKeyboardMarkup(row_width=2)
            m.add(
                btn("Refresh",    style="primary", emoji_key="refresh", callback_data=f"bot_logs:{bid}"),
                btn("Clear Logs", style="danger",  emoji_key="trash",   callback_data=f"bot_clearlogs:{bid}"))
            m.add(btn("Back", style="primary", emoji_key="back", callback_data=f"bot_detail:{bid}"))
            ...
            m = types.InlineKeyboardMarkup(row_width=2)
            m.add(
                btn("Yes, Delete", style="danger",  emoji_key="trash", callback_data=f"bot_confirm_del:{bid}"),
                btn("Cancel",      style="primary", emoji_key="back",  callback_data=f"bot_detail:{bid}"))
            ...
            if bd:
                ...
                fp = bd.get('file_path', '')
                uf_root = user_folder(bd['user_id'])
                try:
                    if fp and os.path.isdir(fp):
                        if os.path.realpath(fp) != os.path.realpath(uf_root):
                            shutil.rmtree(fp, ignore_errors=True)
                        else:
                            entry = bd.get('entry_file', '')
                            if entry:
            ...
            if not bots_list:
                ...
                m2.add(btn("Deploy Bot", style="success", emoji_key="send", callback_data="menu_deploy"))
                m2.add(btn("Main Menu",  style="primary", emoji_key="home", callback_data="go_home"))
                safe_edit(f"📭 <b>No bots yet!</b>\n\nDeploy your first bot!\n📦 Slots: 0/{mx}\n"
                ...
                for b in bots_list:
                    ...
                    m2.add(btn(f"{st_icon2} {b['bot_name'][:15]} — #{b['bot_id']}",
                               style="success" if (r2 and appr2 == 'approved') else "primary",
                               emoji_key="green" if (r2 and appr2 == 'approved') else "robot",
                               callback_data=f"bot_detail:{b['bot_id']}"))
                m2.add(btn("Deploy New Bot", style="success", emoji_key="send", callback_data="menu_deploy"))
                m2.add(btn("Main Menu",      style="primary", emoji_key="home", callback_data="go_home"))
            ...
            disk_quota = 100
            try:
                bdir, ud, disk_quota = _term_dirs_and_quota(bd)
                target = str(bdir) if bdir and Path(bdir).exists() else str(ud)
                disk_used = dir_size_mb(target) if _SECURITY_OK else 0.0
            except Exception:
                pass
            m = types.InlineKeyboardMarkup(row_width=2)
            m.add(
                btn("Refresh", style="primary", emoji_key="refresh", callback_data=f"bot_res:{bid}"),
                btn("Back",    style="primary", emoji_key="back",    callback_data=f"bot_detail:{bid}"))
            safe_edit(
                ...
                f"💾 RAM: {ram} MB\n"
                f"⚡ CPU: {cpu}%\n"
                f"📂 Disk: {disk_used:.1f}/{disk_quota} MB\n"
                f"⏱️ Uptime: {uptime_str}\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"<i>Tip: Refresh once more if CPU still shows 0</i>",
                chat_id, msg_id, reply_markup=m)
            safe_answer(call.id)

        
        elif data.startswith("bot_pip:"):
            bid = data.split(":")[1]
            bd = db.get_bot(bid)
            if not bd:
                return safe_answer(call.id, "Not found!", show_alert=True)
            if bd['user_id'] != uid and not state.is_admin(uid):
                return safe_answer(call.id, "Not yours!", show_alert=True)
            state.set_state(uid, {'action': 'pip_install', 'bid': bid})
            safe_answer(call.id)
            safe_edit(
                f"📦 <b>Pip / Package Install</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"Bot: <code>{bd['bot_name'][:22]}</code>\n\n"
                f"Send package name(s):\n"
                f"• <code>requests</code>\n"
                f"• <code>telebot pillow</code>\n"
                f"• <code>requests==2.31.0</code>\n\n"
                f"<i>Only safe package names allowed.\n"
                f"Installs into this bot folder only.</i>",
                chat_id, msg_id,
                reply_markup=types.InlineKeyboardMarkup().add(
                    btn("Cancel", style="danger", emoji_key="cross",
                        callback_data=f"bot_detail:{bid}")))
        ...
        # ═══ ADMIN STATS ═══
            ...
            safe_edit(
                ...
                f"  CPU: {ss['cpu']}% | RAM: {ss['mem']}%\n"
                f"  Disk: {ss['disk']}%\n"
                f"🛡 Security: {'ACTIVE' if _SECURITY_OK else 'OFF'}\n"
                f"━━━━━━━━━━━━━━━━━━━━",
            ...
            for p in pays[:10]:
                ...
                m.add(
                    btn(f"#{p['payment_id']}", style="success", emoji_key="check",
                        callback_data=f"pay_approve:{p['payment_id']}"),
                    btn(f"#{p['payment_id']}", style="danger",  emoji_key="cross",
                        callback_data=f"pay_reject:{p['payment_id']}"))
            if not pays:
            ...
            m.add(
                btn("7 Days",   style="success", emoji_key="clock",
                    callback_data=f"adm_quicksub:{plan}:{target}:7"),
                btn("30 Days",  style="success", emoji_key="clock",
                    callback_data=f"adm_quicksub:{plan}:{target}:30"),
                btn("90 Days",  style="success", emoji_key="clock",
                    callback_data=f"adm_quicksub:{plan}:{target}:90"))
            m.add(
                btn("180 Days", style="success", emoji_key="clock",
                    callback_data=f"adm_quicksub:{plan}:{target}:180"),
                btn("365 Days", style="success", emoji_key="clock",
                    callback_data=f"adm_quicksub:{plan}:{target}:365"),
                btn("Lifetime", style="success", emoji_key="crown",
                    callback_data=f"adm_quicksub:lifetime:{target}:0"))
            m.add(btn("Cancel", style="danger", emoji_key="cross", callback_data="menu_admin"))
            ...
            for ch in channels:
                t += f"  {'🟢' if ch['is_active'] else '🔴'} @{ch['channel_username']} — {ch['channel_name']}\n"
            if not channels:
                t += "  No custom channels. Default: @sb_aura\n"
            t += "\n━━━━━━━━━━━━━━━━━━━━"
            ...
            if not channels_r:
                t_ch += "  No custom channels. Default: @sb_aura\n"
            t_ch += "\n━━━━━━━━━━━━━━━━━━━━"
            ...
            m = types.InlineKeyboardMarkup()
            m.add(btn("Cancel", style="danger", emoji_key="cross", callback_data="adm_channels"))
            ...
            for sk in list(bot_scripts.keys()):
                ...
                if i:
                    ...
                    if bid_inner:
                        ...
                        try:
                            shutdown_terminal(str(bid_inner))
                        except Exception:
                            pass
            ...
            rn = len([k for k in bot_scripts if is_running(k)])
            try:
                sb_info = sandbox_status() if _SECURITY_OK else {}
            except Exception:
                sb_info = {}
            safe_edit(
                f"🖥 <b>SYSTEM INFO</b>\n"
                ...
                f"🤖 Running Bots: {rn}\n\n"
                f"🛡 <b>Security:</b>\n"
                f"  Sandbox: {sb_info.get('mode', 'N/A')}\n"
                f"  bwrap: {'✅' if sb_info.get('bwrap') else '❌'}\n"
                f"  unshare: {'✅' if sb_info.get('unshare') else '❌'}\n"
                f"━━━━━━━━━━━━━━━━━━━━",
                chat_id, msg_id, reply_markup=back_btn("menu_admin", "Admin"))
            ...
            for ticket in tickets[:10]:
                ...
                m.add(btn(f"Reply #{ticket['ticket_id']}",
                          style="primary", emoji_key="speech",
                          callback_data=f"adm_ticket_reply:{ticket['ticket_id']}"))
            if not tickets:
                t += "No open tickets!"
            m.add(btn("Admin", style="primary", emoji_key="back", callback_data="menu_admin"))
            safe_edit(t + "━━━━━━━━━━━━━━━━━━━━", chat_id, msg_id, reply_markup=m)
            ...
            m = types.InlineKeyboardMarkup()
            m.add(btn("Cancel", style="danger", emoji_key="cross", callback_data="adm_tickets"))
            safe_edit(f"💬 <b>Reply to Ticket #{tid}</b>\n━━━━━━━━━━━━━━━━━━━━\n\n"
                      f"📝 Type your reply now:\n━━━━━━━━━━━━━━━━━━━━",
                      chat_id, msg_id, reply_markup=m)

        elif data == "adm_users" or data.startswith("adm_users_page:"):
            if not state.is_admin(uid):
                return
            safe_answer(call.id)
            page = 0
            if data.startswith("adm_users_page:"):
                try:
            ...
            t = (f"👥 <b>ALL USERS ({s['users']})</b>\n"
                 f"━━━━━━━━━━━━━━━━━━━━\n"
                 f"💎 Subs: {s['active_subs']} | 🚫 Banned: {s['banned']}\n"
                 f"📄 Page {page + 1}/{pages}\n"
                 f"━━━━━━━━━━━━━━━━━━━━\n"
                 f"Tap a user for full details:")

            m = types.InlineKeyboardMarkup(row_width=1)
            for u in chunk:
                icon = "🚫" if u.get('is_banned') else ("💎" if u.get('plan') != 'free' else "👤")
                name = (u.get('full_name') or u.get('username') or str(u['user_id']))[:18]
                plan = u.get('plan', 'free')
                m.add(btn(
                    f"{icon} {name} | {plan}",
                    style="danger" if u.get('is_banned') else "primary",
                    emoji_key="user",
                    callback_data=f"adm_ud:{u['user_id']}"
                ))
            if page > 0 or page < pages - 1:
                nav_btns = []
                if page > 0:
                    nav_btns.append(btn("◀ Prev", style="primary", emoji_key="back",
                                        callback_data=f"adm_users_page:{page - 1}"))
                if page < pages - 1:
                    nav_btns.append(btn("Next ▶", style="primary", emoji_key="play",
                                        callback_data=f"adm_users_page:{page + 1}"))
                m.add(*nav_btns)
            m.add(btn("Admin Panel", style="primary", emoji_key="back", callback_data="menu_admin"))
            safe_edit(t, chat_id, msg_id, reply_markup=m)

        elif data.startswith("adm_ud:"):
            if not state.is_admin(uid):
                return
            safe_answer(call.id)
            try:
                ...
                return safe_edit("❌ Invalid user.", chat_id, msg_id,
                                 reply_markup=back_btn("adm_users", "All Users"))
            u = db.get_user(target_uid)
            if not u:
                return safe_edit(f"❌ User <code>{target_uid}</code> not found!",
                                 chat_id, msg_id, reply_markup=back_btn("adm_users", "All Users"))
            pl = PLAN_LIMITS.get(u.get('plan', 'free'), PLAN_LIMITS['free'])
            bc = db.bot_count(target_uid)
            bots_list = db.get_bots(target_uid)
            running = sum(1 for b in bots_list if bot_running(target_uid, b['bot_name']))
            ...
            mk.add(btn("All Users", style="primary", emoji_key="users", callback_data="adm_users"))
            mk.add(btn("Admin Panel", style="primary", emoji_key="back", callback_data="menu_admin"))
            safe_edit(
                f"👤 <b>User Details</b>\n━━━━━━━━━━━━━━━━━━━━\n\n"
                f"🆔 ID: <code>{target_uid}</code>\n"
                f"📛 Name: {u.get('full_name', '?')}\n"
                ...
                f"📅 Joined: {str(u.get('created_at', '?'))[:16]}\n"
                f"━━━━━━━━━━━━━━━━━━━━",
                chat_id, msg_id, reply_markup=mk)
            ...
            safe_answer(call.id)
            m = types.InlineKeyboardMarkup(row_width=1)
            m.add(btn("Delete Bot Files (disk)",       style="danger", emoji_key="trash",    callback_data="adm_cleanup_files"))
            m.add(btn("Delete Log Files (disk)",       style="danger", emoji_key="trash",    callback_data="adm_cleanup_logs"))
            m.add(btn("Delete Old Backups (keep 5)",   style="danger", emoji_key="trash",    callback_data="adm_cleanup_backups"))
            m.add(btn("Delete Error Logs (DB)",        style="danger", emoji_key="trash",    callback_data="adm_cleanup_errlogs"))
            m.add(btn("Delete Old Notifications (30d)",style="danger", emoji_key="trash",    callback_data="adm_cleanup_notifs"))
            m.add(btn("Admin Panel",                   style="primary", emoji_key="back",    callback_data="menu_admin"))
            safe_edit(
        ...
        # ═══ HELP ═══
            ...
            help_texts = {
                ...
                "bots": "🤖 <b>MANAGING BOTS</b>\n\n• Start/Stop/Restart\n• Toggle Auto Restart (paid plans)\n• Logs & Resources\n• Terminal (isolated shell)\n• Re-detect entry",
                "plans": "💎 <b>PLANS & PRICING</b>\n\n• 🆓 Free — 1 bot — 100MB\n• 🟢 Starter — 2 bots — 500MB — 99 BDT / ⭐ 15\n• ⭐ Basic — 5 bots — 1GB — 199 BDT / ⭐ 30\n• 💎 Pro — 15 bots — 3GB — 499 BDT / ⭐ 75\n• 🏢 Enterprise — 50 bots — 10GB — 999 BDT / ⭐ 150\n• 👑 Lifetime — ♾️ — 50GB — 1999 BDT / ⭐ 400",
...
def cleanup():
    ...
    # Shutdown all isolated terminals
    if _SECURITY_OK:
        try:
            from security import shutdown_all_terminals
...
def main():
    ...
    sb_mode = "Unknown"
    try:
        sb_mode = sandbox_status().get('mode', '?') if _SECURITY_OK else "DISABLED"
    except Exception:
        pass
    ...
    safe_send(OWNER_ID,
        ...
        f"🛡 Sandbox: <b>{sb_mode}</b>\n"
        f"🔍 Scanner: <b>{'ACTIVE' if _SECURITY_OK else 'DISABLED'}</b>\n"
        f"🖥 Terminal: <b>{'ACTIVE' if _SECURITY_OK else 'DISABLED'}</b> (per-bot quota)\n"
        f"💾 Backup: OK\n"
        f"🐙 GitHub Deploy: OK\n"
        f"⭐ Stars Payment: OK\n"
        f"🎟 Promo Codes: OK\n"
        ...
        f"🆓 Free bot limit: {FREE_BOT_MAX_HOURS}h (admin exempt)\n"
