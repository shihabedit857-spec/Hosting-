...
def cleanup_script(sk):
    ...
    try:
        ...
        if bid:
            try:
                from security import shutdown_terminal
...
# ═══════════════════════════════════════════════════
#  BOT RESOURCES
#  cpu_percent(interval=None) returns 0.0 on the FIRST call per process.
#  We prime once, then sample with a short interval so UI shows real CPU.
# ═══════════════════════════════════════════════════
_cpu_primed = set()
...
def bot_res(sk):
    ...
    try:
        ...
        # Prime CPU counters on first sight of this process tree
        pid_key = i['process'].pid
        if pid_key not in _cpu_primed:
            try:
                ...
                pass
            _cpu_primed.add(pid_key)
            # Short blocking sample so first UI open is not stuck at 0
            time.sleep(0.15)
        ...
        # Drop primed pid if process already exited
        try:
            if i['process'].poll() is not None:
                _cpu_primed.discard(pid_key)
        except Exception:
            pass

        return round(mem, 1), round(min(cpu, 999.9), 1)
    except psutil.NoSuchProcess:
        return 0, 0
    except Exception:
        return 0, 0
