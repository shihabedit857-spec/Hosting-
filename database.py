...
class SQLiteDB:
    ...
    def init_tables(self):
        ...
        self.exe("""CREATE TABLE IF NOT EXISTS blocked_hashes(
...
class MongoDB:
    ...
    def add_blocked_hash(self, h, reason='', filename='', user_id=None):
        if not h: return
        try:
            from datetime import datetime
            self.db.blocked_hashes.update_one(
                {'hash': h},
                {'$set': {
                    'hash': h, 'reason': (reason or '')[:200],
    ...
    def is_hash_blocked(self, h):
        if not h: return None
        try:
            return self.db.blocked_hashes.find_one({'hash': h}, {'_id': 0})
        except Exception:
            return None
...
class WrappedSQLite(SQLiteDB):
    ...
    def add_blocked_hash(self, h, reason='', filename='', user_id=None):
        ...
        try:
            self.exe(
                "INSERT OR REPLACE INTO blocked_hashes(hash,reason,filename,user_id) VALUES(?,?,?,?)",
                (h, reason[:200], (filename or '')[:120], user_id))
        except Exception:
            pass

    def is_hash_blocked(self, h):
        if not h: return None
        return self.exe("SELECT * FROM blocked_hashes WHERE hash=?", (h,), one=True)
