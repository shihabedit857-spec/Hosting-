"""BOT RUNNER — uses security.sandbox with per-plan limits"""
...
from config import (
    ...
    ADMIN_LIMITS, PLAN_PROCS, resolve_limits,
)
...
def _get_plan_limits(uid):
    """Fetch effective RAM/CPU/thread limits for a user."""
    try:
        user = _db.get_user(uid) if _db else None
        is_admin = _is_admin_or_owner(uid)
        return resolve_limits(uid, user, is_admin)
    except Exception as e:
        logger.warning(f"_get_plan_limits({uid}) failed: {e}")
        return resolve_limits(uid, None, False)
...
def run_bot_script(bid, cid, att=1):
    ...
    # ── Fetch plan-specific limits ──────────────────────────────────────
    limits   = _get_plan_limits(uid)
    ram_mb   = limits['ram']
    procs    = limits['procs']
        ...
        if _SECURITY_OK:
            try:
                proc = run_sandboxed(
                    ...
                    cpu_sec=cpu_sec,
                    procs=procs)
                logger.info(f"[run] bot #{bid} SANDBOXED ram={ram_mb}MB procs={procs}")
                ...
                proc = subprocess.Popen(
                    ...
                    preexec_fn=_ft.partial(_apply_rlimits, ram_mb, cpu_sec, procs)
